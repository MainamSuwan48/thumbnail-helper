...
let _id = 0;
const uid = () => `col-${++_id}`;
function makeColumn(widthRatio) {
    return {
        id: uid(),
        widthRatio,
        image: null,
...
function makeEqualColumns(count) {
    const ratio = 1 / count;
    return Array.from({ length: count }, () => makeColumn(ratio));
}
export const useBannerStore = create()(persist((set, get) => ({
    canvasWidth: 1280,
    canvasHeight: 720,
    columnCount: 3,
    ...
    setColumnImage: (columnId, image) => set((s) => ({
        columns: s.columns.map((c) => (c.id === columnId ? { ...c, image } : c)),
    })),
    clearColumnImage: (columnId) => set((s) => ({
        columns: s.columns.map((c) => (c.id === columnId ? { ...c, image: null } : c)),
    })),
    updateImagePosition: (columnId, x, y) => set((s) => ({
        columns: s.columns.map((c) => c.id === columnId && c.image ? { ...c, image: { ...c.image, x, y } } : c),
    })),
    updateImageScale: (columnId, scale) => set((s) => ({
        columns: s.columns.map((c) => c.id === columnId && c.image ? { ...c, image: { ...c.image, scale } } : c),
    })),
    updateFilter: (columnId, patch) => set((s) => ({
        columns: s.columns.map((c) => c.id === columnId ? { ...c, filter: { ...c.filter, ...patch } } : c),
    })),
    resizeColumns: (leftId, rightId, delta) => {
        ...
        if (newLeft + newRight !== left.widthRatio + right.widthRatio)
            return;
        set({
            columns: columns.map((c) => {
                if (c.id === leftId)
                    return { ...c, widthRatio: newLeft };
                if (c.id === rightId)
                    return { ...c, widthRatio: newRight };
                return c;
            }),
        });
    },
