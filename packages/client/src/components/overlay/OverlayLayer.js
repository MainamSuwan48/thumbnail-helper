...
import { Layer, Group, Text, Line, Image as KonvaImage, Transformer, } from "react-konva";
...
function BlinkingBorder({ x, y, width, height, }) {
    ...
    return (_jsx(Line, { ref: lineRef, name: "ui-only", points: [x, y, x + width, y, x + width, y + height, x, y + height], closed: true, stroke: "#7c3aed", strokeWidth: 1.5, shadowColor: "#7c3aed", shadowBlur: 8, shadowOpacity: 1, dash: [6, 4], listening: false }));
}
// ── Text group (pic count + "Images" letters) ────────────────────────────────
function TextGroup({ canvasHeight }) {
    const { picCount, selectedOverlayId, setSelectedOverlay } = useOverlayStore();
    const brushMode = useBannerStore((s) => s.brushMode);
    const [imagesCanvas, setImagesCanvas] = useState(null);
    const [extrasCanvas, setExtrasCanvas] = useState(null);
    const imagesRef = useRef(null);
    useEffect(() => {
        document.fonts.ready.then(() => setImagesCanvas(buildImagesCanvas(picCount.fontSize)));
    }, [picCount.fontSize]);
    useEffect(() => {
        if (!picCount.extrasEnabled) {
            setExtrasCanvas(null);
            return;
        }
        document.fonts.ready.then(() => setExtrasCanvas(buildExtrasCanvas(picCount.fontSize, picCount.extrasColor)));
    ...
    const extrasW = picCount.extrasEnabled
        ...
        : 0;
    const totalW = countTextW + (imagesCanvas ? imagesCanvas.width + 10 : 0) + extrasW + 8;
    const totalH = picCount.fontSize + 8;
    return (_jsxs(Group, { x: picCount.x, y: groupY, opacity: picCount.opacity, rotation: -8, scaleX: picCount.scale, scaleY: picCount.scale, onClick: () => {
            if (brushMode || selectedOverlayId === "mascot")
                return;
            setSelectedOverlay("picCount");
            useBannerStore.getState().setSelectedColumn(null);
        }, children: [_jsx(Text, { text: picCount.count, fontFamily: OVERLAY_FONT, fontStyle: "bold", fontSize: picCount.fontSize, fill: picCount.color, stroke: picCount.strokeEnabled ? picCount.strokeColor : undefined, strokeWidth: picCount.strokeEnabled ? picCount.strokeWidth : undefined, fillAfterStrokeEnabled: picCount.strokeEnabled, ...shadow }), imagesCanvas && (_jsx(KonvaImage, { ref: imagesRef, image: imagesCanvas, x: imgX, y: imgY, width: imagesCanvas.width, height: imagesCanvas.height, rotation: -5, listening: false })), picCount.extrasEnabled && (_jsxs(_Fragment, { children: [_jsx(Text, { text: "+", fontFamily: OVERLAY_FONT, fontStyle: "bold", fontSize: plusFontSize, fill: "#ffffff", stroke: "#ffffff", strokeWidth: picCount.strokeEnabled ? Math.max(2, picCount.strokeWidth * 0.5) : 2, fillAfterStrokeEnabled: true, x: plusX, y: picCount.fontSize - plusFontSize + 4, listening: false }), _jsx(Text, { text: picCount.extras, fontFamily: OVERLAY_FONT, fontStyle: "bold", fontSize: picCount.fontSize, fill: picCount.extrasColor, stroke: picCount.strokeEnabled ? picCount.strokeColor : undefined, strokeWidth: picCount.strokeEnabled ? picCount.strokeWidth : undefined, fillAfterStrokeEnabled: picCount.strokeEnabled, x: extrasCountX, ...shadow, listening: false }), extrasCanvas && (_jsx(KonvaImage, { image: extrasCanvas, x: extrasImgX, y: extrasImgY, width: extrasCanvas.width, height: extrasCanvas.height, rotation: -5, listening: false }))] })), isSelected && (_jsx(BlinkingBorder, { x: -4, y: -4, width: totalW, height: totalH }))] }));
}
...
function MascotNode() {
    ...
    const [outlinedCanvas, setOutlinedCanvas] = useState(null);
    ...
    useEffect(() => {
        ...
        if (mascot.outlineEnabled && mascot.outlineThickness > 0) {
            setOutlinedCanvas(createOutlinedCanvas(rawImg, mascot.outlineThickness, mascot.outlineColor));
        }
        else {
            setOutlinedCanvas(null);
        }
    ...
    return (_jsxs(_Fragment, { children: [_jsx(KonvaImage, { ref: imgRef, image: outlinedCanvas ?? rawImg ?? undefined, x: mascot.x - padX, y: mascot.y - padY, width: mascot.width + padX * 2, height: mascot.height + padY * 2, rotation: mascot.rotation, draggable: !useBannerStore.getState().brushMode, onClick: () => {
                    ...
                    node.scaleX(1);
                    node.scaleY(1);
                } }), _jsx(Transformer, { ref: trRef, name: "ui-only", rotateEnabled: true, keepRatio: false, boundBoxFunc: (oldBox, newBox) => newBox.width < 20 || newBox.height < 20 ? oldBox : newBox })] }));
}
// ── Logo (bottom-right, clickable + scalable) ─────────────────────────────────
const LOGO_STROKE_WIDTH = 8;
function LogoImage({ canvasWidth, canvasHeight, }) {
    ...
    useEffect(() => {
        ...
        setOutlinedCanvas(createOutlinedCanvas(htmlImg, LOGO_STROKE_WIDTH, "#ffffff"));
    }, [htmlImg]);
    if (!outlinedCanvas)
        return null;
    const baseW = 500;
    const baseH = Math.round((htmlImg.naturalHeight / htmlImg.naturalWidth) * baseW);
    ...
    return (_jsxs(Group, { children: [_jsx(KonvaImage, { image: outlinedCanvas, x: x - padScale, y: y - padScale, width: logoW + padScale * 2, height: logoH + padScale * 2, onClick: () => {
                    if (brushMode || selectedOverlayId === "mascot")
                        return;
                    setSelectedOverlay("logo");
                    useBannerStore.getState().setSelectedColumn(null);
                } }), isSelected && (_jsx(BlinkingBorder, { x: x - padScale, y: y - padScale, width: logoW + padScale * 2, height: logoH + padScale * 2 }))] }));
}
...
function buildImagesCanvas(countFontSize) {
    ...
    const totalW = charMetrics.reduce((s, m) => s + m.w + gap, 0) + stroke * 2 + 20;
...
function buildExtrasCanvas(countFontSize, color) {
    const fontSize = Math.round(countFontSize * IMAGES_RATIO);
    const stroke = Math.max(2, Math.round(fontSize * STROKE_RATIO));
    const font = `bold ${fontSize}px ${OVERLAY_FONT}`;
    const measure = document.createElement("canvas").getContext("2d");
...
// ── Layer ─────────────────────────────────────────────────────────────────────
export function OverlayLayer({ canvasWidth }) {
    ...
    return (_jsxs(Layer, { ref: layerRef, listening: !brushMode, onClick: (e) => {
            if (e.target instanceof Konva.Layer)
                setSelectedOverlay(null);
        }, children: [_jsx(TextGroup, { canvasHeight: canvasHeight }), _jsx(MascotNode, {}), _jsx(LogoImage, { canvasWidth: canvasWidth, canvasHeight: canvasHeight })] }));
}
